...
class AdminDoctorRecommendation extends StatefulWidget {
  const AdminDoctorRecommendation({super.key});

  @override
  _AdminDoctorRecommendationState createState() => _AdminDoctorRecommendationState();
}

class _AdminDoctorRecommendationState extends State<AdminDoctorRecommendation> {
  ...
  Future<void> _pickAndUploadFile() async {
    ...
    try {
      ...
      await supabase.storage.from('doctor_recommendations').uploadBinary(fileName, fileBytes);


      // Get the public URL
     String fileUrl = supabase.storage.from('doctor_recommendations').getPublicUrl(fileName);


      // Insert file details into Supabase Database
      await supabase.from('doctor_recommendations').insert({
        'user_id': user.id,
        'url': fileUrl,
  ...
  Future<List<Map<String, dynamic>>> _fetchFiles() async {
    final response = await supabase.from('doctor_recommendations').select();
    return response;
  }
  ...
  Future<void> _deleteFile(String fileName, String docId) async {
    try {
      await supabase.storage.from('doctor_recommendations').remove([fileName]);
      await supabase.from('doctor_recommendations').delete().eq('id', docId);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("Doctor Recommendations"), backgroundColor: Colors.pinkAccent),
      body: FutureBuilder<List<Map<String, dynamic>>>(
        future: _fetchFiles(),
        builder: (context, snapshot) {
          if (!snapshot.hasData) return Center(child: CircularProgressIndicator());
          if (snapshot.data!.isEmpty) return Center(child: Text("No recommendations uploaded yet."));
