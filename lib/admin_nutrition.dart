...
class _AdminNutritionScreenState extends State<AdminNutritionScreen> {
  ...
  Future<void> _pickAndUploadFile() async {
    ...
    try {
      ...
      await supabase.storage.from('nutrition').uploadBinary(fileName, fileBytes);

      // Get the public URL
      String fileUrl = supabase.storage.from('nutrition').getPublicUrl(fileName);
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.data!.isEmpty) return Center(child: Text("No nutrition recommendations uploaded yet."));
